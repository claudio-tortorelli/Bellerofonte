...
class UploadFileWithForm extends Single_test implements Serializable {
...
/////////////////////////// BELOW CAN CHANGE FROM TEST TO TEST ----
        ...
        _fileToUpload = (String) parameters.elementAt(3); //read words from file test txt or xml
/////////////////////////// END OF COMMON CHANGE AREA ----
    }
...
/////////////////////////// BELOW CAN CHANGE FROM TEST TO TEST ----
        ...
        rep.addResultExpected("File to upload: " + _fileToUpload);
/////////////////////////// END OF COMMON CHANGE AREA ----
...
/////////////////////////// BELOW CAN CHANGE FROM TEST TO TEST ----
            try {
                ...
                if (form != null) {
                    // perform the upload
                    File file = new File(_fileToUpload);
                    FileInputStream fis = new FileInputStream(file);
                    if (file != null && form.hasParameterNamed(_expectedFieldName)) {
                        UploadFileSpec[] filesToUp = new UploadFileSpec[1];
                        filesToUp[0] = new UploadFileSpec(_fileToUpload, fis, "noContentTypeSpecified");
                        if (filesToUp != null) {
                            form.setParameter(_expectedFieldName, filesToUp);
                        ...
                        } // file specification failed
                        else {
                            rep.addMyResultBad("Unable to create file specification"); //test failed to Report
                            _finalResult = "FAILED"; // don't change: it is used for comparison
                        }
                    } // not good
                    else {
                        if (file == null) {
                            rep.addMyResultBad("Unable to find the file to upload"); //test failed to Report
                            _finalResult = "FAILED"; // don't change: it is used for comparison
                        } else {
                            rep.addMyResultBad("The form hasn't the expected field"); //test failed to Report
                            _finalResult = "FAILED"; // don't change: it is used for comparison
                        }
                    }
                } else {
