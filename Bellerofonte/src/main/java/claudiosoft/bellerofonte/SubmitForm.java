...
class SubmitForm extends Single_test implements Serializable
{
...
/////////////////////////// BELOW CAN CHANGE FROM TEST TO TEST ---- 
  ...
  private String _eventualButton = "";
/////////////////////////// END OF COMMON CHANGE AREA 	
...
/////////////////////////// BELOW CAN CHANGE FROM TEST TO TEST ----	
		...
		_eventualButton= (String)parameters.elementAt(2); //read words from file test txt or xml	  		
/////////////////////////// END OF COMMON CHANGE AREA ----	
 	}
...
/////////////////////////// BELOW CAN CHANGE FROM TEST TO TEST ----
		...
		rep.addResultExpected("Button: " + _eventualButton);
/////////////////////////// END OF COMMON CHANGE AREA ----			
...
/////////////////////////// BELOW CAN CHANGE FROM TEST TO TEST ----
			...
			// one of tried open operation was successfully
				if (form != null)
				{			
				// verify that the action url is active
					String urlAction = form.getAction();
					WebLink act = resp.getLinkWith(urlAction);		
					
				// perform the check						
					if (_eventualButton.equals(""))
					{
						resp = form.submit();
						if (resp != null)
						{
							rep.addMyResultGood("Button found and form submitted"); //test passed to Report								
						...
						{
							rep.addMyResultBad("Button not found and form not submitted"); //test passed to Report								
							_finalResult = "FAILED"; // don't change: it is used for comparison	
						}
					}
					// user have specified which button press
					else
					{
						SubmitButton button = form.getSubmitButton(_eventualButton);						
						if (button == null)
						{
							button = form.getSubmitButtonWithID(_eventualButton);
						}						
						if (button != null)
						{
		...
		// manage the common errors	
			...
			{ 			
				rep.addError("Method not allowed. Error 405.");			
				_finalResult = "ERROR";
			}										
			catch (Exception e)
			{ 			
